	...
	int *pcpu_list)
{
	...
	for (pcpu=0; pcpu<4; pcpu++){
		runq = RUNQ(pcpu);
		printk("\n\t[csched_print] : list of vcpus in runq for pcpu %d : \n", pcpu);
		printk("\t-----------------------------------------------------------\n");
	    	list_for_each( iter, runq )
	    	{
	        	const struct csched_vcpu * const iter_svc = __runq_elem(iter);
			printk("%d | ", iter_svc->vcpu->vcpu_id);
			
	    	}

		printk("\n\t-----------------------------------------------------------\n");
	}
	printk("\t[csched_print] : end of print\n");
	return 0;
}
*/
...
out:
    ...
    update_dom(cpu, snext->vcpu->domain->domain_id);
